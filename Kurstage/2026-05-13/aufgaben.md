# Aufgabe 1

Erstelle eine Tabelle und zugehörige Datenbankklasse `Genre` mit einer `string`-Property `Name`. Diese geht mit der Buch-Klasse/Tabelle eine one-to-many-Beziehung ein.

Lege ein paar Beispiel-Genres in deiner Datenbank an.

Passe die `/AddBook`-Page entsprechend an, so dass der Benutzer dort ein Genre aus der Datenbank auswählen kann.